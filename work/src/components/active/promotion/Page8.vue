<!--
  ...
  -->
<template>
  <section class="active-promotion-page8">
    <!--banner-->
    <div>
      <img class="banner" src="../../../assets/active/promotion/banner8.png"></img>
    </div>
    <!--数据提交表单-->
    <div class="form fs16" v-if="status != 1">
      ...
      <div>
        ...
        <button class="btn sms-btn fr" :class="sms.timeall != sms.timenow && 'disabled'" @click="getSms()">{{ sms.btn }}</button>
      </div>
      <!--登录密码-->
      <div>
        <div class="input-wrap">
      ...
      <!--错误提示-->
      <div class="err-hint tac c-white">{{ err }}</div>
      <!--提交按钮-->
      <div>
        <button :class="!agree && 'disabled'" class="btn page8-btn" v-submit="{cb: submitClick}">立即申请</button>
      </div>
      <!--协议-->
      <div class="protocol-link tac fs14 c-white">
        <i class="agree" :class="agree && 'active'" @click="switchAgree(agree)"></i>
        <span>我已阅读并同意</span>
        <a class="c-link-8" @click="showProtocolPop()">《贷上钱服务协议》</a>
      </div>
    </div>
    <!--三个特点-->
    <div class="feature">
      <img class="fl" src="../../../assets/active/promotion/feature1.png" alt="">
      <img class="fl" src="../../../assets/active/promotion/feature2.png" alt="">
      <img class="fl" src="../../../assets/active/promotion/feature3.png" alt="">
    </div>
    <!--注册成功提示-->
    ...
    <!--公司信息-->
    <div class="company-info tac fs14 c-white">
      <p class="telnum">400-136-5799</p>
      <p class="addr">上海市淞虹路207号明基商务广场C座2层</p>
    </div>
    <!--用户已存在弹框提示-->
...
<script>
  ...
  var $data = {
    ...
    // 是否同意协议
    agree: true,
    // 渠道码和今日头条 js id 的对应关系
    ids: {
      '58ede2fe3c7f': '59037032231',
      '5901a872096a': '59505893298',
      '5901a8603c30': '59505893298',
      '5901a82d8a7d': '59505893298',
      '5901a8f2a35b': '59505893298',
      '5901a888f142': '59505893298',
      // 20170508 add
      '5910140f30f5': '59956805522',
      '05910140f30f5': '59958292188',
      '15910140f30f5': '59958358492',
      '25910140f30f5': '59958356458',
      // 20170510 add
      '5912a954e4': '60070056150',
      '05912a954e4': '60070056150',
      '15912a954e4': '60070056150',
      '25912a954e4': '60070056150',
      '35912a954e4': '60070056150',
      '591287648199': '59505893298',
      '0591287648199': '59505893298',
      '1591287648199': '59505893298',
      // 20170517 add
      '591bbb60bf8b': '60470073179',
      '0591bbb60bf8b': '60470073179',
      '1591bbb60bf8b': '60470073179',
      '2591bbb60bf8b': '60470073179',
      '3591bbb60bf8b': '60470073179',
      '591bbd40290d': '60470073179',
      '0591bbd40290d': '60470073179',
      '1591bbd40290d': '60470073179',
      '2591bbd40290d': '60470073179',
      '3591bbd40290d': '60470073179',
    },
    // 渠道码
        ...
        ajax.post('/v2/user/register', {
          ...
          if (res.code == 0) {
            ...
            if ($data.ids[$data.channel]) {
              ...
              try {
                _taq.push({
                  convert_id: $data.ids[sysinfo.getQueryString('channel')] || 0,
                  event_type: "form"
                })
              } catch (err) {
                console.log(err)
...
<style lang="scss">
  ...
  .active-promotion-page8 {
    ...
    /*提交表单*/
    .form {
      background: linear-gradient(to bottom, #3da5fe, #6839f4);
